    ...
    */
    mapLayers() {
      ...
      const assembledPrevious = () => {
        const fields = [];
        if (this.logs[this.currentLogIndex].geofield.data.length > 0) {
          fields.push(this.logs[this.currentLogIndex].geofield.data[0].geom);
        }
    ...
    logAreas: {
      handler(newAreas, oldAreas) {
        ...
        if (areaDeleted.length > 0
          && this.logs[this.currentLogIndex].geofield.data.length > 0
